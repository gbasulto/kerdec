...
## Uncontaminated gamma sample
X <- rgamma(n, shape = 5, rate = 5)
mean(X)
sd(X)
hist(X, freq = FALSE)

## Generate panel data  (matrix).
panel_smp <- 
  sweep(x = matrix(rnorm(n = 150*4, mean = 0, sd = .2), ncol =  4),
        MARGIN = 1, STATS = X, FUN = "+")

## Limits where function is going to be estimated.
low <- -1
upp <- 6
...
yy <- 
  kerdec_dens_panel_1d_cpp(panel_smp, h = .15, low, upp,
                           resolution, ker = 4)
